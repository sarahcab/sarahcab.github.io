...
SELECT st_astext(geom) from communes_eurostat where comm_id like %(comm_id)s
    ...
    ##Ponctuelle
    cur.execute('''
INSERT INTO communes (id_comm,nbr_chem,nbr_site,geom) VALUES (%(comm_id)s,%(nbrChem)s,%(nbrSit)s,ST_GeomFromText(%(geo)s, %(srid)s))
    ''',{'comm_id':comm, 'nbrChem':nbrChem, 'nbrSit':nbrSit,'geo':newpoint,'srid':srid})
