	...
	d3.select("#bouton_l")
		.on("click",function(){
			...
			setTimeout(function(){
				...
				d3.select("#fleche").transition().delay(2000).duration(650).attr("transform","translate(0,480)").transition().attr("class","clignote")
...
///fonctionnement des évènements au scroll
function scrollanim2(delta){
	if(flechee==false){
		flechee=true;
		d3.select("#fleche").attr("class","").transition().duration(700).attr("transform","translate(0,1500)")//.transition().remove();
	}
	...
	if(delta>0&&av==true){
			...
			if(way<lengthOp){
				...
				d3.select("#roue").attr("opacity",0.5) 
					.attr("transform",function(){
						...
						return "rotate("+(r*1+vit*1*0.5)+" 98.4 98.4)"
					})
		...
		if(scrollTime==0&&way<=0){
			...
			if(way>L-lengthOp){
				...
				d3.select("#lum_pt"+(scrollTime)+"_plus").attr("opacity",0)
				...
				d3.select("#etape_"+scrollTime).attr("opacity",0)
				d3.select("#etape_"+scrollTime).selectAll("circle")
					.attr("r",0)
