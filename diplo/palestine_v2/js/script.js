		...
		if(scrollTime>5&&(way>=L||way==0)){
			...
			if(way<lengthOp){
				...
				if(document.getElementById("point_"+scrollTime)&&scrollTime>=0){
					...
					vb=(vbX[scrollTime-1]*1+(vbX[scrollTime]*1-vbX[scrollTime-1]*1)*((way-L+lengthOp*1)/lengthOp))+" "+(Y0*1+(Y*1-Y0*1)*((way-L+lengthOp*1)/lengthOp)-vbH/2)+" "+vbW+" "+vbH;
					d3.select("#svg_scroll").attr("viewBox",vb)
					trans0=ls_trans[scrollTime-1]
					trans=ls_trans[scrollTime]
					d3.select("#informations_etape").attr("transform","scale(0.85) translate("+(trans0[0]*1+(trans[0]*1-trans0[0]*1)*((way-L+lengthOp*1)/lengthOp))+","+(trans0[1]*1+(trans[1]*1-trans0[1]*1)*((way-L+lengthOp*1)/lengthOp))+")")
				}
			} else {
