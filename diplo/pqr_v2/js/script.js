...
var nuancierChoro = ["#BAADCC","#D2CB96","#95916D","#605F42"]; 
var valsChoro = [2,4,6];
var itDate =0;
var vitAnim = 1500;
var dragX = 0;
...
function dragWheel(){
	...
	.on("drag", function(d) {
		...
		///maj par rapport aux QUARTS DE CERCLES : voir dessn repangle dans util
		
		//sens trigo
		// if(posY-centrey<0){ //
			// if(posX-centrex<0){
				// angle=180-angle;
				// d3.select("#testcoul").attr("stroke","yellow")
			// } else {
				// angle = angle*(-1)
		...
		// } else {
			// if(posX-centrex<0){
				// angle = 180+angle*(-1);
				// d3.select("#testcoul").attr("stroke","blue")
			// } else {
				// angle = 360-angle;
		...
		var add = (parseFloat(angle)+360*tours)-ancAngle; //c dans le sens trigo
		
		if((posY-centrey)*ancY<0&&(posX-centrex)>0){
			if(add>0){
				// alert("moins")
		...
		angleOk = parseFloat(angle)+360*tours;
		var addDate = parseFloat(angleOk)-ancAngle; //maj de add
		///
		
		///mis à jour des variables repère
		if(ancAngle==d.a){
			angle0=angle;
			d3.select("#indic_reculer").attr("fill","red")
		} else {
			d3.select("#indic_reculer").attr("fill","blue")
		}
		ancAngle = angleOk;
		ancY = posY-centrey;
		///
		...
		// date = parseFloat(date)+addDate/5.625;  //beug : l'angle est négatif pas par rapport au sens
		date = 1945 + parseInt(angleOk/45);
		// date = parseInt(date);
		var position = angleOk-angle0; 
		d3.select("#indic_avancer").text(date+" | "+angle0+" | "+tours); //" | "+angle+" | "+trans+
	...
	.on("dragend", function(d){
		d.a = angleOk;
		// date = parseFloat(date)+parseFloat(angle%360)/180;  //beug : l'angle est négatif pas par rapport au sens
		// d3.select("#indic_avancer")
		// alert(d.a)
		// calc();
		// majGraph();
		// majVilles();
	...
	if(itTest%2==0){
		if(bool==false){
			...
			.attr("transform",function(){
				...
				return "rotate("+(indic%360)*quot*vit+" "+cx+" "+cy+")"
			})
		}
...
function initMap(){
	...
	function callback0(error, geoDepts, geoVilles, dataT, dataQ){ //geo pays - geo depts - villes - tirage - noms quotidiens 
		...
		var map = d3.select("#graph") 
			...
			// .style("margin-left","53%")
			// .style("overflow","visible")
			.attr("transform","scale("+width/widthGraph+") translate("+(widthGraph*0.53*widthGraph/width)+",-10)")
