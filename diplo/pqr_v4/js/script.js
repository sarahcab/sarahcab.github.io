...
scale = 11000, //échelle de la carte

nuancier = ["#0E1A4F","#0D470D","#000000","#FCD96D", "#00b0eb","#afca0a"], //titres locaux, titres nationaux,contours boules et legende, survol locaux(!!dans le css, class hightligth), tirage locaux, tirage nationaux
nuancierChoro = ["#AAAAAA","#a2d1f4"], 
...
mode_legende = true;
defile=false; //défilement continu
...
function actions(){ //animation et interactivités
	...
	d3.select("#bout_partie1")
		.on("click",function(){
			...
			d3.selectAll(".partie2").transition().duration(1000).attr("transform",function(){
				return "translate(0,3000)"
			})
			.transition()
			.duration(500)
			...
			d3.select("#ind_date")
				.transition()
				.duration(500)
				.attr("transform","matrix(1 0 0 1 916.2786 155.1807)")
		})
		.on("mouseover",function(){
			d3.select("#lab_partie1").transition().duration(500).attr("opacity",1)
		})
		.on("mouseout",function(){
			d3.select("#lab_partie1").transition().duration(500).attr("opacity",0)
		})	
		
	d3.select("#bout_partie2")
		.on("click",function(){
			...
			d3.select("#ind_date")
				.transition()
				.duration(500)
				.attr("transform","matrix(1 0 0 1 900 180)")
		})
		.on("mouseover",function(){
			d3.select("#lab_partie2").transition().duration(500).attr("opacity",1)
		})
		.on("mouseout",function(){
			d3.select("#lab_partie2").transition().duration(500).attr("opacity",0)
		})
	...
	d3.select("#reculer").on("click",function(){
		...
		d3.select("#a_cont").attr("class","mdSimple cliquable").attr("opacity",1); //on rend forcément 'avancer' cliquable
	...
	d3.select("#avancer").on("click",function(){
		...
		if(itDate==listeDates.length-1){ 
			...
			d3.select("#a_cont").attr("class","mdSimple").attr("opacity",0.2); //on vire cliquable lorsque c'est la date maximale
		}
		date = listeDates[itDate]; //mise à jour de la date par rapport à la liste de dates commentées
	...
	d3.select("#a_cont")
		...
		.on("click",function(){
			if(defile==false){
				vitAnim=300;
				d3.select("#avancer").attr("class","mdSimple").attr("opacity",0.2)
				d3.select("#reculer").attr("class","mdSimple").attr("opacity",0.2)
				d3.selectAll(".ttt").attr("opacity",0)
				d3.select(this).select("polygon").transition().duration(300).attr("fill","#FFFFFF")
				repeat = setInterval(function(){ ac() }, 300)
				defile=true;
			} else {
				defile=false;
		// })
		// .on("mouseup",function(){
				d3.select(this).select("polygon").transition().duration(300).attr("fill","#3F1112")
				if(date<anneeMax){
				...
				d3.selectAll(".ttt").attr("opacity",1)
				d3.select("#drag0").attr("opacity",1)
	...
	function callback0(error, geoDepts, geoVilles, dataT, dataQ, dataTxt){ //recap : geo depts - villes - tirage - noms quotidiens - textes
		...
		var gVilles = map.selectAll(".gVille") //groupe contenant les socles et les ronds des quotidiens : à transformer en barre
			...
			.on("mouseover",function(){
				...
				d3.select("#ville_affquot").text(ville);
				affVille(ville);
			})
			.on("mouseout",function(){
				ville = this.attributes.Ville.value;
				d3.select("#ville_affquot").text("");
				delVille(ville);
			})
			.attr("id", function(d){
				return "gr_"+d.Ville
		...
		//trait corse
		p1 = projection([8.4,43.7])
		p2 = projection([6.4,42.4])
		map.append("line")
			.attr("x1",p1[0])
			.attr("y1",p1[1])
			.attr("x2",p2[0])
		...
		//espace survol
		var indvX = (document.getElementById("rep_affquot").attributes.transform.value).split(" ")[4]; //récupéré dans les repères
		var indvY = ((document.getElementById("rep_affquot").attributes.transform.value).split(" ")[5]).split(")")[0]; //idem : ! transform de type matrix
		...
		buildReps(); //construction de la liste des années par groupe de 8
...
function buildGraph(){		
	...
	var pasAnnee = 5; //indication des années
	...
	var dragp2 = d3.behavior.drag() //fonction "drag" veut dire que tu fais bouger l'objet en l'attrapant
		.on("drag", function(d) {
			min = 0;
			max = w*(anneeMax-anneeMin)/ampli;
			d.x += d3.event.dx;
			if(d.x<min){
				d.x = min;
			} else if(d.x>max){
			...
			date=parseInt((d.x*ampli/w)+anneeMin*1);
			vitAnim=0;
			calc();
			majGraph();
		})
	...
	d3.select("#svgCourbes")
		.selectAll(".suiv") //petits cercles qui suivent les courbes et la date
		.data([["ci_Lt",nuancier[0],YLt,dataTir[0].LOC_Nb_titres],["ci_Nt",nuancier[1],YNt,dataTir[0].NAT_Nb_titres],["ci_Lr",nuancier[4],YLr,dataTir[0].LOC_Tirages*100000],["ci_Nr",nuancier[5],YNr,dataTir[0].NAT_Tirages*100000]])
		.enter()
		.append("rect")
		.attr("id",function(d){
			return d[0]
		...
		.on("mouseover",function(){
			d3.select(this)
				.attr("opacity",0.8)
				.attr("x",function(d){
					val = this.attributes.x0.value;
					return val-8;
				})
				.attr("y",function(d){
					val = this.attributes.y0.value;
					return val-8;
				})
				.attr("width",40)
				.attr("height",40)
		...
		.on("mouseout",function(){
			d3.select("#fond_val").remove();
			d3.select("#tx_val").remove();
			d3.select(this)
				.attr("opacity",1)
				.attr("x",function(d){
					val = this.attributes.x0.value;
	...
	for(j=0;j<ampli;j+=pasAnnee){ //bouclage dans l'amplitude en fonction du bas (/2 pour les repères non-légendé, tous les 5 ans)
		// if(j%2==0){
...
function buildReps(){ //construction de slistes pour les repères autour de la roue
	ls_temp = []
	for(i=anneeMin;i<anneeMax*1+1;i++){
		ls_temp.push(i);
		if((i-anneeMin)%8==7){
			anneesR.push(ls_temp);
			ls_temp = [];
		}
	}
}
...
function majGraph(){ //mise à jour des courbes
	...
	var listeVals= [valLt,valNt,parseInt(valLr*100000),parseInt(valNr*100000)]; //liste des hauteurs pour les cercles : on inverse car on va du haut vers le bas
	...
	d3.select("#bouboule") //on déplace la barre selon la date
		.transition()
		.duration(vitAnim)
		.selectAll("circle")
		.attr("cx",vB)
...
function delQuot(ville,n,obj){ //init de la fonction précédente
	d3.selectAll(".hightlight").attr("class","dept").attr("stroke","") //département d'aire de diffusion
	...
	d3.select("#pleins").remove();
	d3.select("#contourss").remove();
	...
	d3.select("#ind_main").text("")
	d3.select("#ind_apres").text("")
