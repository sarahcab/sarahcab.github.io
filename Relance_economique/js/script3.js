...
function resize(){
	...
	var val3 = widthPop/65;

	d3.select("body").selectAll(".adapte").style("font-size",val+"px")
	d3.select("body").selectAll(".adapte2").style("font-size",val2+"px")
	d3.select("body").selectAll(".adapte3").style("font-size",val3+"px")
}
...
function appendBarre(){ //construction de la barre
	...
	// d3.select("#barretime").append("text").attr("id","titreBarre")
		// .attr("x",sizeTw/2-80)
		// .attr("y", sizeTh+parseFloat(debY)+3)
...
function boutdemo(){
	...
	d3.select(".interlibre").style("display","none");
	d3.select(".interdemo").style("display","block");
	d3.select("#barretime").style("display","block");
	d3.select("#titreBarre").style("display","block");
	...
	d3.select("#txt_n11").select("text").attr("transform","matrix(1 0 0 1 315 483.5)"); //mauvaises dimensiosn et dessins pas à la meme place
...
function timedCount() {   
    ...
    t = setTimeout(function(){ timedCount() }, vitesse);
}
function rien(){
...
function suivant(){ //continuer/lancer
	...
	if(continuue==true){ //permet de vérifier que l'on n'a pas fait de pause manuelle : si ct le cas, on répèterait deux fois la même anumation
		var tot = listeNoeuds[ind].length;
		for(l=0;l<tot;l++){
...
function fin(){ //se déclenche à la fin de la lecture
	...
	etapepause.style.display = "none";	
	d3.select("#term").style("display","block"); //indication qui sra suprrimée au clic sur suivant
}
//animations
function maj2(id, csvData){
	var tot = csvData.length;
	for(j=0; j<tot; j++){
		if (csvData[j].r == id){
...
function utlibre(){
	...
	d3.select(".interlibre").style("display","block");
	d3.select(".interdemo").style("display","none");
	d3.select("#soundcontrol").style("display","none");
	d3.select("#barretime").style("display","none");
	d3.select("#titreBarre").style("display","none");
	d3.select("#video").style("display","none");
	d3.select("#playch").style("display","none");
	//décalage du titre de noeud 11 (création d'emplois et donc placement d'une ristourne 
	d3.select("#ristourne").attr("opacity",0);
	d3.select("#txt_n11").select("text").attr("transform","matrix(1 0 0 1 315 488)");
	//cache qui rend les éléments non cliquables
