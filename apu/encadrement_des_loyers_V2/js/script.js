...
function fonctionnalites(){
	...
	d3.select("#sources_infos")
		.on("click",function(){
			if(this.innerHTML == "Plus d'infos ici..."){
				this.innerHTML = "(Masquer)";
				d3.select("#plus_infos").style("display","block")
			}else{
				this.innerHTML = "Plus d'infos ici...";
				d3.select("#plus_infos").style("display","none")
			}
		})
}
...
function maj_compare(){
	...
	if(parseFloat(loyerm2)<parseFloat(loyerRefMinore)){
		...
		console.log("jadot");
		d3.select("#reponse").html("Ca passe encore").style("color",nuancier_resultat[2]);
		d3.select("#reponse_detail").html("Ton loyer est en dessous du loyer de référence majoré").style("color",nuancier_resultat[2]);
		d3.select("#loyer_trop").html("");
		...
		d3.select("#loyer_trop").style("color",nuancier_resultat[3]).html("Le loyer est trop élevé de <span style='font-weight:800'>"+loyerTrop.toFixed(2)+" €</span>. Ton propriétaire a l'obligation de justifier ce complément de loyer").style("color",nuancier_resultat[3]);
